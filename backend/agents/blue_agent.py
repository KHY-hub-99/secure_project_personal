...
class BlueDefenseResult(BaseModel):
    response: str = Field(
        description="The final conversational text that the user will see. Act as the chatbot persona."
    )
...
class BlueAgent:
    ...
    def get_defense_system_prompt(self, category: str) -> str:
        """
    ...
    def generate_defense(self, category:str, attack_payload: str) -> str:
        """
        방어 기준에 따라 공격에 대해 챗봇이 거절 또는 이행하는 텍스트를 반환합니다.
        """
        sys_prompt = self.get_defense_system_prompt(category)
        ...
        if self.llm_client:
            ...
            result: BlueDefenseResult = self.llm_client.chat(
                messages=messages, 
                role="blue", 
                max_tokens=2048,
                response_model=BlueDefenseResult
            )
            
            if result:
                return result.response
            else:
                return "Error generating defense (Pydantic parsing failed)."
